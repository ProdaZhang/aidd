# AIDD toolchain (references/scripts)

The **deterministic scripts** bundled with the AIDD methodology. Two main lines:
...
> Tool 1 = the **UI-capture phase** (`references/phases/ui-capture.md`, not a script): reads a screenshot into a UI DSL (`.md`). The grammar contract is in `../ui-dsl-spec.md`. The scripts below consume this DSL.
...
- **finalize-phase admission**: before finalizing always run `config_check` + `value_check`; if there's a major → send it back.
...
**Screenshot into the library**: `the UI-capture phase` (tool 1) → `<screenID>.md` → `ui_render` (restore for verification) + `ui_palette --merge` (sample colors) [+ `ui_slice` (split assets)] → into `patterns/ui-patterns/`.
