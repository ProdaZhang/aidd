...
`Draft` → `Prototyping` → `Final` → `Recheck` (marked by reverse lookup after a shared item changes) → back to `Final` after the recheck.
...
> **`Recheck` exit**: recheck passes → status returns to `Final`, and `Final time` is refreshed to this recheck's confirmation time (this column = "most recent finalization/recheck"); fails → bounced back (recorded in table E). The executor = the **sync** step (it's the one that marks Recheck, and also the one that clears it).
...
|------|------|-----------------|------|--------------------------|
| `<…>` | S01 | Final → Prototyping | handoff exposed a design flaw | S07 (Final → Recheck) |
