...
def test_norm_status_strips_star_and_emphasis():
    ...
    assert M._norm_status("**Prototyping**") == "Prototyping"
...
_CLEAN = """# manifest
## status enum
`Draft` -> `Prototyping` -> `Final` -> `Recheck`.
...
|------|------|-----------------|------|------------|
| 2026-06-17 | S01 | Final* -> Prototyping | x | S02 |
"""
...
def test_integration_pilot_no_major():
    ...
    path = os.path.join(root, ".uploads", "aidd-pilot", "manifest.md")
